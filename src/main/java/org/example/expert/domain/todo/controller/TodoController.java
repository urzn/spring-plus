...
public class TodoController {
    ...
    public ResponseEntity<Page<TodoResponse>> getTodos(
        ...
        LocalDateTime finishDateTime = (finishDate != null) ? finishDate.plusDays(1).atStartOfDay() : null;

        if(startDate != null && finishDate != null && startDate.isAfter(finishDate)){
            throw new InvalidRequestException("검색 기간의 시작이 끝 이후일 수 없습니다.");
        }

        return ResponseEntity.ok(todoService.getTodos(page, size, weather, startDateTime, finishDateTime));
    }
